...
class ObliqueDecisionTree:
    def __init__(self, max_depth=8, min_sample_split=2, solver="liblinear", random_state=42, loss="gini", C=1.0):
        """
        ...
        Parameters:
            ...
            solver (str): The solver to use for the logistic regression. Default is "liblinear".
            random_state (int): The random state for the logistic regression. Default is 42.
            loss (str): The loss function to use for the decision tree. Default is "gini". "entropy" is another option given.
            C (float): The regularization strength for the logistic regression. Default is 1.0.
        """
        self.root = None
    ...
    def _build_tree(self, X: np.ndarray, y: np.ndarray, depth: int) -> TreeNode:
        ...
        """
        if depth >= self.max_depth or len(set(y)) == 1 or len(y) < self.min_sample_split:
        ...
        coefficients = LogisticRegression(
            solver=self.solver, random_state=self.random_state, max_iter=1000, C=self.C, penalty='l1').fit(X, y).coef_[0]
        ...
        if np.sum(left_indices) < self.min_sample_split or np.sum(right_indices) < self.min_sample_split:
            majority_label = np.bincount(y.astype(int)).argmax()
            return TreeNode(is_leaf=True, majority_label=majority_label)
    ...
    def _find_optimal_threshold(self, sorted_projections: np.ndarray, sorted_labels: np.ndarray) -> float:
        ...
        for i in range(1, n):
            ...
            if self.loss == "entropy":
                left_impurity = self._entropy(left_labels)
                right_impurity = self._entropy(right_labels)
            else:
...
def train_pruned(
    ...
    tree = ObliqueDecisionTree(max_depth=12, min_sample_split=20,
                               solver="liblinear", random_state=1334, loss="gini", C=0.1)
    tree.fit(X, y)
...
def test(
    train_file: str, val_file: str, test_file: str, max_depth: int, output_file: str, weights_file: str
) -> None:
    """
    ...
    tree = train_pruned(train_file, val_file, max_depth, weights_file)
...
def main():
    ...
    predictions = sys.argv[5]
    weights = sys.argv[6]
    test(train_file, val_file, test_file, 12, predictions, weights)
